...
public class CanRelayBridgeDiscoveryService extends AbstractDiscoveryService {
    ...
    private final Logger logger = LoggerFactory.getLogger(CanRelayBridgeDiscoveryService.class);

    private SerialPortManager serialPortManager;

    public CanRelayBridgeDiscoveryService() throws IllegalArgumentException {
    ...
    protected void startScan() {
        ...
        portEnum.forEach(portIdentifier -> {
            ...
            if (portIdentifier.isCurrentlyOwned()) {
                ...
                properties.put("serialPort", portName);
                ThingUID bridgeUID = new ThingUID(THING_TYPE_HW_CAN_BRIDGE, Integer.toHexString(portName.hashCode()));
                thingDiscovered(DiscoveryResultBuilder.create(bridgeUID)
                        .withLabel("@text/thing-type.canrelay.hwBridge.label").withProperties(properties).build());
            }
        });
